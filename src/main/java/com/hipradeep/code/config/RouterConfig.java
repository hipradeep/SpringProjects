...
public class RouterConfig {
    ...
    @RouterOperations({
            ...
            @RouterOperation(path = "/router/customers/stream", produces = {
                    MediaType.TEXT_EVENT_STREAM_VALUE }, method = RequestMethod.GET, beanClass = CustomerStreamHandler.class, beanMethod = "getCustomers", operation = @Operation(operationId = "getCustomersStream", responses = {
                            @ApiResponse(responseCode = "200", description = "successful operation", content = @Content(schema = @Schema(implementation = Customer.class)))
                    })),
            @RouterOperation(path = "/router/customers/{input}", produces = {
            ...
            @RouterOperation(path = "/router/customers/save", produces = {
